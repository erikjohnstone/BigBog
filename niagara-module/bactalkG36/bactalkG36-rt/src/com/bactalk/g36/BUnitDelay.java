...
import com.bactalk.g36.kernel.UnitDelay;
...
/** CDL.Discrete.UnitDelay: the input sampled one period earlier. */
public final class BUnitDelay extends BKernelComponent {
  ...
  public static final Type TYPE = Sys.loadType(BUnitDelay.class);

  private UnitDelay kernel;
  ...
  protected void rebuildKernel() {
    kernel = new UnitDelay(seconds(getSamplePeriod()), getInitialValue().getDouble());
  }
  ...
  protected boolean isParameter(Property property) {
    return property == samplePeriod || property == initialValue;
  }
