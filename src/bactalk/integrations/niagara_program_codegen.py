...
def _trim_and_respond_standalone_source(class_name: str, config: dict[str, Any]) -> str:
    ...
    expected = "time requestCount deviceOn hold" if hold_enabled else "time requestCount deviceOn"
    hold_parse = (
        "\n      boolean hold = Double.parseDouble(args[index + 3]) != 0.0;" if hold_enabled else ""
    )
...
def _trim_and_respond_program_source(config: dict[str, Any], execute_period_seconds: int) -> str:
    ...
    hold_enabled = bool(config["hold_enabled"])
    variant = "have_hol=true" if hold_enabled else "have_hol=false"
    status_check = "\n        || !getHold().getStatus().isOk()" if hold_enabled else ""
...
def _boolean_initialization_standalone_source(class_name: str, config: dict[str, Any]) -> str:
...
def _timer_with_reset_standalone_source(class_name: str, config: dict[str, Any]) -> str:
...
def _timer_with_reset_program_source(config: dict[str, Any], execute_period_seconds: int) -> str:
...
def _accumulating_timer_standalone_source(class_name: str, config: dict[str, Any]) -> str:
...
def _accumulating_timer_program_source(config: dict[str, Any], execute_period_seconds: int) -> str:
...
def _numeric_first_order_hold_config(block: Block) -> dict[str, Any]:
    if block.kind != BlockKind.NUMERIC_FIRST_ORDER_HOLD:
        raise NiagaraProgramCodegenError(
            f"ProgramObject generator does not support {block.kind.value}"
        )
    if block.config.get("semantic_contract") != "CDL.Discrete.FirstOrderHold":
        raise NiagaraProgramCodegenError(
            "numeric_first_order_hold generation requires the explicit "
            "CDL.Discrete.FirstOrderHold contract"
        )
    return {
        "sample_period_seconds": float(block.config["sample_period_seconds"]),
...
def _numeric_first_order_hold_kernel_members(config: dict[str, Any]) -> str:
    period = _java_number(config["sample_period_seconds"])
    return f"""  private static final double SAMPLE_PERIOD_SECONDS = {period};
  private boolean initialized = false;
  private double t0 = 0.0;
  ...
  public double step(double timeSeconds, double input) {{
    ...
    if (!initialized) {{
      t0 = Math.rint(Math.floor(timeSeconds / SAMPLE_PERIOD_SECONDS)
          * SAMPLE_PERIOD_SECONDS * 1.0e6) / 1.0e6;
    ...
    double output = due
        ? uSample
        : preUSample + slope * (timeSeconds - tSample);
    if (due) {{
      ...
      uSample = input;
      preUSample = previous;
      slope = timeSeconds <= t0 + SAMPLE_PERIOD_SECONDS / 2.0
          ? 0.0
          : (input - previous) / SAMPLE_PERIOD_SECONDS;
    }}
    previousTimeSeconds = timeSeconds;
    return output;
  }}
...
def _numeric_first_order_hold_standalone_source(class_name: str, config: dict[str, Any]) -> str:
    return f"""// Generated by BACTalk. Exact CDL.Discrete.FirstOrderHold kernel.
public final class {class_name} {{
{_numeric_first_order_hold_kernel_members(config)}
  public static void main(String[] args) {{
    if (args.length == 0 || args.length % 2 != 0) {{
      throw new IllegalArgumentException("expected groups: time input");
    }}
    {class_name} hold = new {class_name}();
    for (int index = 0; index < args.length; index += 2) {{
      System.out.println(Double.toString(hold.step(
          Double.parseDouble(args[index]), Double.parseDouble(args[index + 1]))));
    }}
  }}
}}
...
def _numeric_first_order_hold_program_source(
    config: dict[str, Any], execute_period_seconds: int
) -> str:
    members = _numeric_first_order_hold_kernel_members(config).replace(
        "  private static final", "private static final"
    )
    for declaration in ("boolean", "double", "long"):
        members = members.replace(f"  private {declaration}", f"private {declaration}")
    members = members.replace("  public double", "public double")
    return f"""// Generated by BACTalk from the pinned CDL.Discrete.FirstOrderHold contract.
Clock.Ticket ticket;
private static final int EXECUTE_PERIOD_SECONDS = {execute_period_seconds};
private long programStartedMillis = 0L;
...
def _boolean_sample_trigger_config(block: Block) -> dict[str, Any]:
    if block.kind != BlockKind.BOOLEAN_SAMPLE_TRIGGER:
        raise NiagaraProgramCodegenError(
            f"ProgramObject generator does not support {block.kind.value}"
        )
    if block.config.get("semantic_contract") != "CDL.Logical.Sources.SampleTrigger":
        raise NiagaraProgramCodegenError(
            "boolean_sample_trigger generation requires the explicit "
            "CDL.Logical.Sources.SampleTrigger contract"
        )
    return {
        "period_seconds": float(block.config["period_seconds"]),
        "shift_seconds": float(block.config["shift_seconds"]),
        "semantic_contract": "CDL.Logical.Sources.SampleTrigger",
    }


def _boolean_sample_trigger_kernel_members(config: dict[str, Any]) -> str:
    period = _java_number(config["period_seconds"])
    shift = _java_number(config["shift_seconds"])
    return f"""  private static final double PERIOD_SECONDS = {period};
  private static final double SHIFT_SECONDS = {shift};
  ...
  public boolean step(double timeSeconds) {{
    ...
    double phase = SHIFT_SECONDS
        - Math.floor(SHIFT_SECONDS / PERIOD_SECONDS) * PERIOD_SECONDS;
    long index = (long) Math.floor(
        (timeSeconds - phase) / PERIOD_SECONDS + 1.0e-9);
    boolean fired = index > lastIndex;
    if (fired) {{ lastIndex = index; }}
    previousTimeSeconds = timeSeconds;
    return fired;
  }}
"""
...
def _boolean_sample_trigger_standalone_source(class_name: str, config: dict[str, Any]) -> str:
    return f"""// Generated by BACTalk. Exact CDL.Logical.Sources.SampleTrigger kernel.
public final class {class_name} {{
{_boolean_sample_trigger_kernel_members(config)}
  public static void main(String[] args) {{
    if (args.length == 0) {{
      throw new IllegalArgumentException("expected times");
    }}
    {class_name} trigger = new {class_name}();
    for (String argument : args) {{
      System.out.println(Boolean.toString(trigger.step(Double.parseDouble(argument))));
    }}
  }}
}}
"""
...
def _boolean_sample_trigger_program_source(
    config: dict[str, Any], execute_period_seconds: int
) -> str:
    members = _boolean_sample_trigger_kernel_members(config).replace(
        "  private static final", "private static final"
    )
    members = members.replace("  private long", "private long")
    members = members.replace("  private double", "private double")
    members = members.replace("  public boolean", "public boolean")
    return f"""// Generated by BACTalk from the pinned CDL.Logical.Sources.SampleTrigger contract.
Clock.Ticket ticket;
private static final int EXECUTE_PERIOD_SECONDS = {execute_period_seconds};
private long programStartedMillis = 0L;
...
public void onExecute() throws Exception {{
  try {{
    double timeSeconds = Math.max(
        0.0, (System.currentTimeMillis() - programStartedMillis) / 1000.0);
    getOut().setValue(step(timeSeconds));
  }} finally {{ scheduleNext(); }}
}}
...
def _plant_equipment_availability_config(block: Block) -> dict[str, Any]:
    ...
    contract = "Buildings.Templates.Plants.Controls.StagingRotation.EquipmentAvailability"
    if block.config.get("semantic_contract") != contract:
        raise NiagaraProgramCodegenError(
...
def _plant_equipment_availability_standalone_source(class_name: str, config: dict[str, Any]) -> str:
...
def _plant_enable_config(block: Block) -> dict[str, Any]:
    ...
    return {
        ...
        "schedule": [[float(row[0]), float(row[1])] for row in block.config["schedule"]],
        "outdoor_lockout": float(block.config["outdoor_lockout"]),
        "outdoor_lockout_hysteresis": float(block.config["outdoor_lockout_hysteresis"]),
        "ignored_requests": int(block.config["ignored_requests"]),
        "minimum_state_time_seconds": float(block.config["minimum_state_time_seconds"]),
...
def _plant_enable_kernel_members(config: dict[str, Any]) -> str:
  ...
  private static final boolean HAVE_INPUT_SCHEDULE = {str(config["have_input_schedule"]).lower()};
  private static final double[] SCHEDULE_TIMES = new double[] {{{times}}};
  private static final double[] SCHEDULE_VALUES = new double[] {{{values}}};
  private static final double OUTDOOR_LOCKOUT = {_java_number(config["outdoor_lockout"])};
...
def _plant_enable_program_source(config: dict[str, Any], execute_period_seconds: int) -> str:
...
def _numeric_changed_config(block: Block) -> dict[str, Any]:
    ...
    return {
        "initial": float(block.config.get("initial", 0.0)),
        "semantic_contract": str(block.config.get("semantic_contract", "CDL.Numeric.Change")),
    }
...
def _numeric_changed_program_source(config: dict[str, Any], execute_period_seconds: int) -> str:
    initial = _java_number(config["initial"])
...
def _plant_stage_completion_standalone_source(class_name: str, config: dict[str, Any]) -> str:
...
def _plant_stage_index_kernel_members(config: dict[str, Any]) -> str:
  ...
  private static final double MINIMUM_RUNTIME = {_java_number(config["minimum_runtime_seconds"])};
  private int stage = 0;
...
def _plant_stage_index_standalone_source(class_name: str, config: dict[str, Any]) -> str:
...
def _plant_stage_index_program_source(config: dict[str, Any], execute_period_seconds: int) -> str:
...
def _plant_hrc_enable_config(block: Block) -> dict[str, Any]:
    ...
    return {
        ...
        "minimum_state_time_seconds": float(block.config["minimum_state_time_seconds"]),
        "sufficient_load_time_seconds": float(block.config["sufficient_load_time_seconds"]),
        "temperature_limit_1_time_seconds": float(block.config["temperature_limit_1_time_seconds"]),
        "temperature_limit_2_time_seconds": float(block.config["temperature_limit_2_time_seconds"]),
        "semantic_contract": contract,
    }
...
def _plant_hrc_enable_standalone_source(class_name: str, config: dict[str, Any]) -> str:
...
def _plant_hrc_enable_program_source(config: dict[str, Any], execute_period_seconds: int) -> str:
...
def _plant_hrc_mode_kernel_members(config: dict[str, Any]) -> str:
    return f"""  private static final double HEATING_COP = {_java_number(config["heating_cop"])};
...
def _plant_hrc_mode_program_source(config: dict[str, Any], execute_period_seconds: int) -> str:
...
def _program_assets(
    ...
    if block.kind in _KERNEL_PROGRAMS:
        ...
        contract = {
            ...
            "recurrence": (
                ...
                + (", minimum hold duration, and sample-boundary release" if hold_enabled else "")
            ),
        }
        ...
        contract = {
            "standard": ("Buildings.Templates.Plants.Controls.Utilities.TimerWithReset"),
    ...
    elif block.kind == BlockKind.NUMERIC_FIRST_ORDER_HOLD:
        config = _numeric_first_order_hold_config(block)
        class_name = f"CdlNumericFirstOrderHold_{digest[:12]}"
        source = _numeric_first_order_hold_program_source(config, execute_period_seconds)
        kernel = _numeric_first_order_hold_standalone_source(class_name, config)
        inputs = [("in", "b:StatusNumeric")]
        output_type = "b:StatusNumeric"
        contract = {
            "standard": "Buildings.Controls.OBC.CDL.Discrete.FirstOrderHold",
            "runtime_oracle": "Open Control Engine",
            "recurrence": (
                "extrapolate from the last two fixed-period samples using the stored "
                "slope and the source-defined first-trigger initialization"
            ),
        }
    elif block.kind == BlockKind.BOOLEAN_SAMPLE_TRIGGER:
        config = _boolean_sample_trigger_config(block)
        class_name = f"CdlBooleanSampleTrigger_{digest[:12]}"
        source = _boolean_sample_trigger_program_source(config, execute_period_seconds)
        kernel = _boolean_sample_trigger_standalone_source(class_name, config)
        inputs = []
        output_type = "b:StatusBoolean"
        contract = {
            "standard": "Buildings.Controls.OBC.CDL.Logical.Sources.SampleTrigger",
            "runtime_oracle": "Open Control Engine",
            "recurrence": (
                "emit one true scan when each phase-shifted periodic boundary is "
                "first reached, then retain the last fired boundary index"
            ),
        }
        ...
        source = _plant_equipment_availability_program_source(config, execute_period_seconds)
        ...
        source = _plant_stage_completion_program_source(config, execute_period_seconds)
