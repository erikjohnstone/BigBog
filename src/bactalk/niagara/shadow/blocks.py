...
KERNEL_BINDINGS: tuple[KernelBinding, ...] = (
    ...
    KernelBinding(
        "IntegratorWithReset",
        "IntegratorWithReset",
        (("in", NUMERIC), ("resetValue", NUMERIC), ("trigger", BOOLEAN)),
        (("out", NUMERIC),),
        (("gain", "gain", _D, 1.0), ("initial", "initialValue", _D, 0.0)),
        source_at_start=True,
        resample=True,
    ),
    KernelBinding(
        "OnCounter",
        "OnCounter",
        (("trigger", BOOLEAN), ("reset", BOOLEAN)),
        (("out", NUMERIC),),
        (("initial", "initialValue", _D, 0.0),),
        source_at_start=True,
        resample=True,
    ),
    KernelBinding(
        "WetBulb",
        "WetBulb",
        (("dryBulb", NUMERIC), ("relativeHumidity", NUMERIC)),
        (("out", NUMERIC),),
        (),
    ),
    KernelBinding(
