...
function BoptestQualificationPanel({ job, graph, canQualify, otherQualificationActive, onQualify, qualifying, onCancel, canceling }: { job?: QualificationJob; graph?: ControlGraph; canQualify: boolean; otherQualificationActive: boolean; onQualify: (qualification: BoptestQualification) => Promise<unknown>; qualifying: boolean; onCancel: (jobId: string) => void; canceling: boolean }) {
  ...
  const authoring = useMemo(() => {
    try {
      return { qualification: buildBoptestQualification(selectedTestCase, graphInputs, graphOutputs, measurementBindings, actuatorBindings, oracleDrafts, steps, stepSeconds, startTime, warmupPeriod, scenarioDraft), message: null };
    } catch (reason) {
...
type TrajectoryCounterexample = {
  violation_count: number;
  first_violation_time: number;
  last_violation_time: number;
  peak_error_time: number;
  peak_absolute_error: number;
  window: { start_index: number; end_index: number; test_times: number[]; test_values: number[] };
};

function OracleCounterexample({ counterexample }: { counterexample?: TrajectoryCounterexample | null }) {
  if (!counterexample) return null;
  return <div className="oracle-counterexample"><CircleAlert size={14} /><div><strong>Minimal failing window</strong><span>{counterexample.violation_count} violation{counterexample.violation_count === 1 ? '' : 's'} · {formatDuration(counterexample.first_violation_time)}–{formatDuration(counterexample.last_violation_time)} · peak error {formatNumber(counterexample.peak_absolute_error)} at {formatDuration(counterexample.peak_error_time)}</span><code>samples {counterexample.window.start_index + 1}–{counterexample.window.end_index + 1}: {counterexample.window.test_values.map(formatNumber).join(' → ')}</code></div></div>;
}
