...
function CtrlFlowConfigurator({ catalog }: { catalog: Catalog }) {
  ...
  const sequenceReview = useMutation({ mutationFn: () => {
    ...
    if (!reconciliation.data?.ready_for_sequence_reconciliation) throw new Error('Retain a passing contractor point reconciliation before review.');
    return api.approveCtrlFlowSequenceRequirements(templateId, effectiveSelections, sequenceFile, reconciliation.data.point_reconciliation_id, {
  ...
  return <div className="ctrl-flow-configurator">
    <aside className="ctrl-flow-controls">
      ...
      <div className="ctrl-flow-upload"><FileSpreadsheet size={18} /><label><strong>{pointsFile?.name || 'Contractor points list'}</strong><small>CSV or XLSX · checked against this design</small><input accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => { setPointsFile(event.target.files?.[0] ?? null); reconciliation.reset(); sequenceReview.reset(); oracleApproval.reset(); setOracleDrafts({}); setFacetOracleDrafts({}); }} type="file" /></label><button disabled={!pointsFile || reconciliation.isPending} onClick={() => reconciliation.mutate()} type="button">{reconciliation.isPending ? 'Checking…' : 'Check points'}</button></div>
    ...
    <section className="ctrl-flow-results">
      ...
      {sequenceReconciliation.data && <div className={`ctrl-flow-sequence-result ${sequenceReconciliation.data.language_coverage_complete ? 'ready' : 'blocked'}`}>
        ...
        {reviewItems.length > 0 && <section className="ctrl-flow-review" aria-label="Engineer requirement review">
          ...
          <button className="ctrl-flow-review-submit" disabled={!reviewComplete || !reconciliation.data?.ready_for_sequence_reconciliation || sequenceReview.isPending} onClick={() => sequenceReview.mutate()} type="button">{sequenceReview.isPending ? 'Re-deriving and validating…' : `Submit ${reviewItems.length} decisions`}<ChevronRight size={15} /></button>
          {sequenceReview.data && <div className={`ctrl-flow-review-outcome ${sequenceReview.data.ready_for_independent_oracle_authoring ? 'ready' : 'blocked'}`}><strong>{sequenceReview.data.ready_for_independent_oracle_authoring ? `${sequenceReview.data.oracle_draft_count + sequenceReview.data.authorable_manual_facet_count} local test trajectories ready for independent authoring` : 'Requirement review remains blocked'}</strong><span>Retained review {sequenceReview.data.review_id.slice(0, 12)} · artifact {sequenceReview.data.retention.artifact_digest.slice(0, 12)} · {sequenceReview.data.scenario_oracle_gap_count} required scenario facets still lack executable oracles · graph generation remains disabled</span>{sequenceReview.data.blockers.length > 0 && <ul>{sequenceReview.data.blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul>}</div>}
