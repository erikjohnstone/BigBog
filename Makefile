...
# The native .bog pipeline contract (GOAL-NATIVE-BOG.md): units, validator,
# lowering matrix, emitter, point linking, Shadow Runtime. Must pass on every commit.
test-native-bog:
	PYTHONPATH=src .venv/bin/python -m bactalk.niagara.lowering --check
	PYTHONPATH=src .venv/bin/python scripts/build_calibration_kit.py --check
	.venv/bin/pytest -m native_bog

# Regenerate the Gate G-WB calibration kit under calibration/ (committed).
calibration-kit:
	PYTHONPATH=src .venv/bin/python scripts/build_calibration_kit.py
...
# Compile the bactalkG36 kernels and component wrappers (needs a JDK; no Niagara SDK).
kernels-check:
	PYTHONPATH=src .venv/bin/python -m bactalk.niagara.kernels --check

# Shadow Runtime: the Python kernel ports against the Java kernels, row by row
# (needs a JDK; docs/decisions/007).
shadow-check:
	PYTHONPATH=src .venv/bin/python -m bactalk.niagara.shadow check

# Shadow-run an exported .bog against a job's acceptance suite:
#   make shadow-run BOG=out/program.bog JOB=out/job.json
shadow-run:
	PYTHONPATH=src .venv/bin/python -m bactalk.niagara.shadow run $(BOG) --job $(JOB)

# Regenerate the Niagara type/slot catalog and the known-bad validator fixtures
# from the vendored corpus (needs make bootstrap-full).
niagara-catalog:
	PYTHONPATH=src .venv/bin/python scripts/harvest_niagara_catalog.py
