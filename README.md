...
BACTalk is a human-gated programming workbench for building controls: the controls-industry analogue of an AI coding agent. It turns a controls job into a typed program, emits a native Niagara `.bog` (an exact ProgramObject source-and-wiring package survives only behind a per-job expert flag), tests the behavior, and exposes the proposed artifact and evidence for engineering approval.
...
- Six deterministic Niagara-targeted prototype/qualifying packs: a bounded `G36_VAV_REHEAT` standard VAV pack (a proportional subset, not Guideline 36; the LBNL controller lane is the G36 path), AHU safety/cooling, AHU duct-static PI control, exhaust-fan command/proof alarm, fail-closed two-pump duty/standby selection, and the 38-model LBNL plant-controls library.
...
- A native Niagara `.bog` pipeline (GOAL-NATIVE-BOG.md, milestones N0–N11 done, N8 partial; `STATUS.md` is the record): a type/slot catalog harvested from 51 real archives with a 16-rule static validator that every produced `.bog` must pass; a lowering matrix that classifies all 58 IR kinds (stock exact, stock within documented bands, `bactalkG36` module, unsupported) and is rendered to `docs/niagara-lowering-matrix.md` from code; the `bactalkG36` Niagara module (13 plain-Java kernels pinned to the OCE goldens and the IR interpreter, component wrappers compiled against `javax.baja` stubs in CI, Gradle skeleton; building and signing is Gate G-SDK); and a direct-XML emitter that exports the retained LBNL VAV reheat and multizone AHU controllers as one foldered, laid-out, validated `.bog` each with SVG previews; and point linking that reads a contractor station as data, suggests proxy points for a human to confirm in the Normalize step, and writes the links into the assembled station (one AHU plus 25 VAVs in the fixture) with no generated Java; and a Shadow Runtime (`python -m bactalk.niagara.shadow run program.bog --job job.json`) that executes the exported file itself with Niagara-like semantics (status bits, priority arrays, event-driven links, a simulated clock, the `bactalkG36` kernels through a JVM sidecar or their checked Python ports), runs every acceptance scenario under several plausible execution orderings, and lists each semantic assumption with its source in `docs/niagara-semantics.md`; and the N7 proofs: a three-way differential (Shadow Runtime, IR interpreter, retained Open Control Engine reference) inside documented pyfunnel bands, hypothesis-generated IR graphs compiled and executed both ways, a `.bog` mutator with eight operators judged by validator, loader, suite and differential (catch rates recorded as measured in `artifacts/native-bog/d4-tier1.json`), a Shadow Runtime qualification job whose `bog-simulated` evidence joins the approval digest and appears in the web app beside BOPTEST and Alfalfa, and a committed calibration kit (`calibration/`) a human runs once in Workbench to mark every assumption VERIFIED or CONTRADICTED. Nothing in this lane claims Niagara runtime qualification (Gate G-WB). N8 extends the lane to Tier 2: `bactalk.library_tier2` carries every LBNL Guideline 36 equipment and zone controller as a configuration (controller plus complete parameter set), plus twelve G36 chiller-plant rows translated from a second locked LBNL checkout and labelled pre-release (plant enable, plant reset, supply setpoints, head pressure, minimum-flow bypass, and the waterside economizer, chilled- and condenser-water pumps, towers, staging setpoints and the stage-up and stage-down processes; the full `Plants.Chillers.Controller` is listed with the algebraic loop the engine finds in LBNL's own wiring), and Tier 2b: all 38 `Templates.Plants.Controls` controllers through the plain CDL lane, 22 retained and 16 listed with their blocker (15 are or contain Modelica-equation or StateGraph utilities the engine cannot execute; one needs a `TimeTable` schedule BACTalk does not map yet; docs/decisions/015). Each retained row has a retained translation, mechanically generated scenarios and a retained Open Control Engine reference whose trajectories are the expectations; `docs/coverage.md` (from `make coverage-report`, also at `GET /api/library/coverage` and on the Libraries page) grades every configuration D1–D4 and names the exact blocker for the ones the toolchain refuses; cross-equipment request signals (zone requests summed at the AHU, AHU requests at the plant) are generated from the G36 request table and built into the assembled station as kitControl chains. N9 adds the Test Generation Protocol for Tier 3 and up (`bactalk.protocol`, docs/decisions/011): a sequence with no LBNL reference starts as a numbered, cited requirement set (data with a digest); the test author builds every scenario from it mechanically and is barred from the logic by its import list; adequacy is proven by the suite, decision coverage, invariants on 10 000 randomised sequences, the Shadow Runtime differential and mutation, all reported as measured; failures are classified and only ambiguous requirements become questions; and a qualified engineer approves the exact digest (Gate G-ENG, Libraries → Requirements) before a candidate can be approved or exported. The first Tier 3 item is the hot water plant (`bactalk.library_tier3`, two boilers, two pumps, minimum-flow bypass, reset, staging, rotation, alarms) with its readable test plan in `docs/test-plans/`. N10 adds Tier 4 (`bactalk.library_tier4`: exhaust fan, duty/standby pumps, rooftop unit, heat pump, DOAS as BACTalk standard sequences, configurable by declared options and never labelled G36) and N11 Tier 5 (`bactalk.protocol.custom`: a contractor's specification section drafted into requirements by the AI, approved, then drafted into a program from the approved requirements only, tested under the same protocol and labelled custom, job-specific; a fixture composes two such sequences with the Tier 1 AHU and VAV box in one `.bog`).
...
- A capability/evidence UI that distinguishes discovered, installed, executable, product-wired, target-compiled, bog-simulated, verified, field-qualified, and production-supported stages. `bog-simulated` means the exported `.bog` itself ran in the Niagara Shadow Runtime against its source of truth; it sits below `verified` because the runtime's Niagara assumptions are unconfirmed until a licensed Workbench runs the calibration kit (Gate G-WB).
...
Open [http://127.0.0.1:8000](http://127.0.0.1:8000). The workbench walks every candidate through five stages: **Intake** (guided source-package normalization, or the ctrl-flow design pipeline at `/intake/design`), **Build** (a typed wiresheet with catalog-declared ports, live value chips, flow on wires, and ghost diffs for AI proposals), **Test** (master timeline, trend panes, boolean lanes, decision and fault coverage, the qualification matrix, and the Simulation Center for BOPTEST, Alfalfa, and the virtual BACnet lab, all on one scrubbable clock), **Review** (every evidence surface acknowledged, then an approval bound to the exact artifact digest, or a rejection with a reason), and **Release** (the server's release summary and the hand-off to a licensed Workbench, downloads only after approval). Projects (`/projects`) put a whole building on the same clock: the system map, cross-program signal bindings carrying live values, project acceptance evidence, optional 3D massing, project approval, and the whole-building intake at `/projects/new`. Libraries, Connections, and Administration expose the pinned reference stack, every virtual BACnet lab with its point grid and probe, the audit chain, and the integration use audit. Dark and light themes are both first-class. Generated runs live under `.bactalk/runs/` and are ignored by git.

The retired static workbench remains at [http://127.0.0.1:8000/legacy/](http://127.0.0.1:8000/legacy/) for one transition period; `/next/` keeps older links working and serves the same React bundle as `/`. To develop and verify the React/TypeScript frontend:
...
The build regenerates TypeScript API types from FastAPI's OpenAPI document before compiling, then checks the bundle budget (`npm run size`: the entry chunk stays under 350 kB gzip and the canvas runtimes load lazily). `npm run test:coverage` enforces coverage thresholds on the trace engine, the stores, and the semantic diff. Browser tests (`npm run e2e`) cover desktop and tablet layouts with an accessibility audit on every page, the real intake-normalization path, the design pipeline, the simulation center with a mocked job stream, digest-bound approval and rejection, whole-building projects, and the legacy workbench boundary; `e2e/visual.spec.ts` keeps pixel baselines of the wiresheet, schematic, and trends at a fixed cursor with reduced motion.
...
FastAPI also exposes interactive API documentation at `/docs` before the root static mount in development integrations. The React workbench is served at `/` (and `/next/` for older links); the retired static workbench is served at `/legacy/`.
