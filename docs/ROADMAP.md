...
- Carry approved ctrl-flow artifacts into generation through the fail-closed candidate preflight. **Implemented through immutable evidence-chain verification, parameter and converter gates, typed graph boundary binding, target assessment, and replay of every approved oracle. Recursive exact-prefix composite assembly now resolves the configured full G36 multizone AHU through OCE and lowers it to a complete typed/source target. Candidate retention/diff, broader parent-controller qualification, exact contractor bindings, and licensed runtime proof remain.**
