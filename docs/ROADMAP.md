...
- Expand the product-wired ctrl-flow Linkage Schema lane beyond its current multizone AHU and VAV terminal templates. **The three current templates now map into exact LBNL G36 controller IDs, component inventories, conditional point contracts, qualification scenarios, capability candidates, and explicit release blockers; contractor points are reconciled against every generated contract with exact/audited aliases and unit-conversion evidence. Exact point-list bytes, mappings, configuration, actor/tenant, and hashes are retained in append-only records, and sequence approval must reference a passing record with the same template and selections. Uploaded sequences receive source-hashed, excerpt-backed facet coverage across every selected scenario and unknown scenario types fail closed. Numeric thresholds, comparisons, units, timing, actions, output values, policies, and clause relationships are extracted into unapproved structured candidates with ambiguous/incompatible point bindings and vague terms retained. Approver-only exhaustive review, source/digest re-derivation, explicit ambiguous point choice, and non-executable oracle drafting are implemented in the API and UI; vague text must be revised and re-uploaded. Completed reviews and exact source bytes are append-only local artifacts with source/candidate/review/artifact digest verification and tenant-filtered authenticated retrieval. Independent oracle authoring and approval are also implemented: distinct identity, exhaustive draft coverage, SI/IP conversion, false→true→false conditions, no-early-trip timing, immutable required outcomes, recovery, real `AcceptanceCase` timelines, and retained approval. Source-bound manual facet trajectories cover Boolean, mode, and event behavior that has no numeric extraction; exact equality, typed I/O, scenario-specific audited input/output allowlists, changed trigger state, controlled output response, and both input/output recovery are mandatory, while unrelated points and unmentioned source behavior cannot be used as bypasses. A source-overlap traceability matrix keeps whole-system generation closed until every selected scenario facet has an executable oracle; local clause approval alone cannot unlock it. The current 44-variant audit proves 2,229 required-point obligations and 767 complete scenario I/O contracts. External immutable retention, consuming approved oracles in automatic graph planning, compiling approved briefs into complete verified target jobs, and adding more upstream templates remain.**
- Expand the compiler test matrix across Niagara versions and module sets.
- Carry approved ctrl-flow artifacts into generation through the fail-closed candidate workflow. **Implemented through immutable evidence-chain verification, parameter and converter gates, complete typed graph-boundary binding, output-to-oracle coverage, target assessment, replay of every approved oracle, and retention as a normal signed candidate run. Recursive exact-prefix composite assembly resolves the configured full G36 multizone AHU through OCE and lowers it to a complete typed/source target. Broader controller-family qualification, source-package compilation in licensed Workbench, runtime proof, hardware-in-the-loop, and field qualification remain.**
...
- Tier-2 BOPTEST REST adapter with reproducible test-case containers. **The pinned local stack, real FMU lifecycle, complete typed-graph mapping, bounded overrides, visual qualification authoring, native peak/typical-day and price selection, seeded weather uncertainty, exact scenario readback, elapsed-time oracle clock, and retained queued closed-loop trajectories are implemented. Qualification attaches atomically to a normal run, enters its signed digest/review bundle, and blocks approval on failure. Production equipment maps and authoritative long-horizon reference suites remain.**
...
- Loopback BACnet/IP fake-building generation, independent BAC0 probing, and mapped acceptance result capture. **Implemented. Reviewed valve/damper models additionally execute command-to-position motion, proofs, leakage/flow curves, persistent mismatch, command loss, stuck/travel/feedback/proof faults, and recovery through real BACnet objects. Automatic signed scenario authoring for physical faults, networked building physics, licensed Niagara attachment, and exact MS/TP hardware remain.**
