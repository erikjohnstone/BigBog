...
2. **Plan** — a sequence pack or AI planner emits `ControlGraph` plus traceable assumptions. For supported system templates, the ctrl-flow adapter first invokes the pinned upstream Linkage Schema interpreter to expose only currently applicable engineering choices, reject invalid selections, and retain evaluated configuration evidence. A deterministic product binding then expands the selected AHU or terminal configuration into components, conditional point requirements, required fault/safety/recovery scenarios, the exact matching LBNL G36 controller, installed capability candidates, and release blockers. Contractor points are reconciled against that conditional contract. Sequence prose is separately reconciled against every scenario, with excerpt evidence, and its thresholds, units, timing, actions, policies, and clause relationships are emitted only as unapproved structured candidates; ambiguous point bindings and vague terms remain blocking. Exhaustive human reviews are append-only artifacts containing the original uploaded bytes and selected configuration, verified on every read against source, candidate, decision, and complete-artifact hashes. Authenticated reads are tenant-filtered. A different engineer then supplies complete baseline, trigger, pre-expiration, post-expiration, and recovery stimuli/expectations. The server validates unit conversions and false→true→false conditions, derives duration-crossing scan counts, prevents required outcomes from being weakened, emits real `AcceptanceCase` timelines, and retains the approval separately. Boolean, mode, and event facets without extracted math use a source-bound manual trajectory whose typed input and output must both change and recover; missing source language cannot be bypassed. A scenario-to-oracle matrix requires every selected facet—not merely every extracted clause—to have executable evidence before whole-system graph generation is permitted. This never permits deployment. Local hashes are tamper-evident consistency checks, not a substitute for external immutable retention.
...
5. **Test** — Tier 1 interprets the same IR against a deterministic plant. Every scan can also become a loopback-only BACpypes3 fake building; its runner drives mapped inputs and captures controller outputs over BACnet/UDP. Tier 2 runs pinned building-physics services locally. `BoptestGraphRunner` requires a complete explicit graph-to-FMU map, feeds each measurement snapshot into the same typed IR interpreter, bounds outgoing commands against advertised inputs, advances the FMU, records KPIs, and stops it in a `finally` path. `WorkbenchService.qualify_with_boptest` executes the exact signed run graph, grades independent trajectory oracles with pyfunnel, atomically appends the runtime evidence and all comparison CSVs, recomputes the artifact digest, and changes the run to failed when any oracle escapes its funnel. The Alfalfa lane first performs bounded, non-executing FMI archive inspection and exposes only declared inputs/outputs plus aggregate internal-variable counts. Simulation Center then requires a reviewed visual binding for every typed graph boundary, including transforms, command bounds, optional echoes, and startup substitutions. `WorkbenchService.qualify_with_alfalfa` admits that FMU and map, executes the same typed graph against the whole-building model under an external clock, verifies command echoes and clean shutdown, and signs both the uploaded model and trajectory. Equipment-pack-specific scenario coverage and failure minimization remain. The final tier connects the generated station in a licensed Niagara lab to those isolated buildings.
